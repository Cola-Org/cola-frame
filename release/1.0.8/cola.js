...
(function() {
    ...
    Element.prototype._set = function(attr, value, ignoreError) {
      ...
      if (typeof value === "string" && this._scope) {
        if (value.charCodeAt(0) === 123) {
          parts = cola._compileText(this._scope, value);
  ...
  cola._compileText = function(scope, text) {
    ...
    while ((s = text.indexOf("{{", p)) > -1) {
      ...
      if (exprStr) {
        ...
        expr = cola._compileExpression(scope, exprStr, exprStr.indexOf(" in ") > 0 ? "repeat" : void 0);
        if (!parts) {
          parts = [expr];
        } else {
          parts.push(expr);
  ...
  cola._compileExpression = function(scope, exprStr, specialType) {
    var aliasName, exp, i;
    if (!exprStr) {
      return null;
    }
    if (exprStr.charCodeAt(0) === 63) {
  ...
  cola.Expression = (function() {
    function Expression(exprStr) {
      ...
      this.writeable = (this.type === "MemberExpression" || this.type === "Identifier") && !this.hasComplexStatement;
      if (this.writeable) {
        this.writeablePath = this.paths[0];
      }
      if (this.hasDynaPath) {
        this.hasComplexStatement = this.hasDynaPath;
        delete this.paths;
      }
      if (watchPaths) {
    ...
    Expression.prototype.compile = function(exprStr) {
      ...
      this.script = parts.join("");
    };

    Expression.prototype.getData = function(scope, path, loadMode, dataCtx) {
      var retValue;
    ...
    Expression.prototype.getParentPathInfo = function() {
      var i, info, path;
      if (this.parentPath !== void 0) {
        return this.parentPath;
      }
      if (this.writeable) {
        path = this.writeablePath;
  ...
  _getEntityPath = function() {
    ...
    this._pathCache = path = path.reverse();
    return path;
  };
    ...
    if (this._watchers) {
      ...
      for (p in ref) {
        ...
        if (p === "**") {
          ...
          shouldTrigger = path.length === holder.path.length;
        } else {
          pv = holder.path;
  ...
  cola.EntityList = (function(superClass) {
    ...
    EntityList.prototype._notify = function(type, arg) {
      ...
      if (this._disableObserverCount === 0) {
        if ((ref = this._dataModel) != null) {
          ref.onDataMessage(this.getPath(), type, arg);
        }
  ...
  cola.Scope = (function() {
    ...
    Scope.prototype._getAction = function(name) {
      var fn, ref;
      fn = this.action[name];
      if (fn == null) {
        fn = (ref = this.parent) != null ? ref._getAction(name) : void 0;
      }
      return fn;
    };
    ...
    Scope.prototype.hasExBinding = function() {
      return this._hasExBinding;
    };

    Scope.prototype.setHasExBinding = function(hasExBinding) {
      var ref;
      if (this._hasExBinding === hasExBinding) {
        return;
      }
      this._hasExBinding = hasExBinding;
      if (hasExBinding) {
        if ((ref = this.parent) != null) {
          ref.setHasExBinding(true);
        }
      }
    };
  ...
  cola.Model = (function(superClass) {
    ...
    function Model(name, parent) {
      ...
      this.action = function(name, action) {
        var a, config, n, store;
        store = this.action;
        if (arguments.length === 1) {
          if (typeof name === "string") {
            return this._getAction(name) || cola.defaultAction[name];
    ...
    Model.prototype.processMessage = function(bindingPath, path, type, arg) {
  ...
  cola.ItemsScope = (function(superClass) {
    ...
    ItemsScope.prototype._setItems = function(items) {
      this.items = items;
      this.originItems = items instanceof Array ? items.$origin : null;
    };

    ItemsScope.prototype.refreshItems = function() {
    ...
    ItemsScope.prototype.processMessage = function(bindingPath, path, type, arg) {
      ...
      if (allProcessed) {
        ...
        itemScope = this.findItemDomBinding(arg.data || arg.entity);
        if (itemScope) {
          itemScope.processMessage(bindingPath, path, type, arg);
        } else if (this.hasExBinding()) {
  ...
  cola.AbstractDataModel = (function() {
    ...
    AbstractDataModel.prototype.bind = function(path, processor) {
      ...
      if (path) {
        this._bind(path, processor);
      }
      return this;
    };

    AbstractDataModel.prototype._bind = function(path, processor) {
      var l, len1, node, nodePath, part, subNode;
    ...
    AbstractDataModel.prototype.unbind = function(path, processor) {
      ...
      if (path) {
        this._unbind(path, processor);
      }
      return this;
    };

    AbstractDataModel.prototype._unbind = function(path, processor) {
      var l, len1, node, part;
      node = this.bindingRegistry;
      for (l = 0, len1 = path.length; l < len1; l++) {
        part = path[l];
        if (part.charCodeAt(part.length - 1) === 35) {
      ...
      if (node != null) {
        return delete node.__processorMap[processor.id];
      }
    };
    ...
    AbstractDataModel.prototype.processDataMessage = function(node, path, type, arg, notifyChildren) {
      ...
      if (notifyChildren) {
        ...
        for (part in node) {
          ...
          if (subNode && (part === "**" || notifyChildren2) && part !== "__processorMap" && part !== "__path") {
            this.processDataMessage(subNode, path, type, arg, true);
          }
        }
      }
    };
  ...
  cola.ItemDataModel = (function(superClass) {
    ...
    ItemDataModel.prototype.setIndex = function(index, silence) {
      this._index = index;
      if (!silence) {
        this.onDataMessage([cola.constants.REPEAT_INDEX], cola.constants.MESSAGE_PROPERTY_CHANGE, {
          entity: null,
  ...
  cola.ElementAttrBinding = (function() {
    function ElementAttrBinding(element1, attr1, expression1, scope) {
      ...
      this.watchingMoreMessage = !this.paths.length && this.expression.hasComplexStatement && !this.expression.hasDefinedPath;
      ref = this.paths;
      for (l = 0, len1 = ref.length; l < len1; l++) {
        path = ref[l];
  ...
  cola.defaultAction.get = function(path) {
  ...
  _ExpressionDataModel = (function(superClass) {
    ...
    _ExpressionDataModel.prototype.processMessage = cola._EMPTY_FUNC;
  ...
  cola.util.userData = function(node, key, data) {
    var i, id, k, store, text, userData, v;
    if (node.nodeType === 3) {
      return;
    }
  ...
  if (cola.browser.ie && cola.browser.ie < 9) {
    setTimeout(function() {
      ...
      setInterval(function() {
        var c, id, len, node, store, text, userData;
        if (cola.util.userDataStore.size < 256) {
          return;
        }
        userData = cola.util.userDataStore;
        c = 0;
  ...
  $(function() {
    setTimeout(function() {
      ...
      if (path) {
        ...
        if (router) {
          cola.setRoutePath((router.redirectTo || cola.setting("defaultRouterPath") || "") + location.search, true, true);
        }
      }
    }, 0);
  });
  ...
  cola._BindingFeature = (function() {
    ...
    _BindingFeature.prototype.clone = function() {
      var cloned, p, v;
  ...
  cola._ExpressionFeature = (function(superClass) {
    ...
    _ExpressionFeature.prototype.init = function(domBinding, force) {
      if (force || this.expressionStr.charCodeAt(0) !== 63) {
        this.prepared = true;
        this.compile(domBinding.scope);
      }
    };

    _ExpressionFeature.prototype.compile = function(scope) {
      var expression;
      this.prepared = true;
      expression = this.expression = cola._compileExpression(scope, this.expressionStr, this.expressionType);
  ...
  cola._RepeatFeature = (function(superClass) {
    ...
    function _RepeatFeature() {
      return _RepeatFeature.__super__.constructor.apply(this, arguments);
    }

    _RepeatFeature.prototype.expressionType = "repeat";

    _RepeatFeature.prototype.ignoreBind = true;

    _RepeatFeature.prototype.compile = function(scope) {
      _RepeatFeature.__super__.compile.call(this, scope);
      return this.alias = this.expression.alias;
    };

    _RepeatFeature.prototype.init = function(domBinding, force) {
      var scope;
      _RepeatFeature.__super__.init.call(this, domBinding, force);
      if (!this.prepared) {
        return;
      }
  ...
  cola._EventFeature = (function(superClass) {
    ...
    _EventFeature.prototype.init = function(domBinding, force) {
      _EventFeature.__super__.init.call(this, domBinding, force);
      if (!this.prepared) {
        return;
      }
  ...
  cola._DomFeature = (function(superClass) {
    ...
    _DomFeature.prototype.writeBack = function(domBinding, value) {
      var ref;
      if (!(this.prepared && ((ref = this.expression) != null ? ref.writeable : void 0))) {
        return;
      }
      this.ignoreMessage = true;
      domBinding.scope.set(this.expression.writeablePath, value);
  ...
  cola._DomClassFeature = (function(superClass) {
    ...
    function _DomClassFeature() {
      return _DomClassFeature.__super__.constructor.apply(this, arguments);
    }
  ...
  cola._TextBoxFeature = (function(superClass) {
    ...
    _TextBoxFeature.prototype.init = function(domBinding, force) {
      var feature;
      _TextBoxFeature.__super__.init.call(this, domBinding, force);
      if (!this.prepared) {
        return;
      }
      feature = this;
  ...
  cola._CheckboxFeature = (function(superClass) {
    ...
    _CheckboxFeature.prototype.init = function(domBinding, force) {
      var feature;
      _CheckboxFeature.__super__.init.call(this, domBinding, force);
      if (!this.prepared) {
        return;
      }
      feature = this;
  ...
  cola._RadioFeature = (function(superClass) {
    ...
    _RadioFeature.prototype.init = function(domBinding, force) {
      _RadioFeature.__super__.init.call(this, domBinding, force);
      if (!this.prepared) {
        return;
      }
      domBinding.$dom.on("click", function() {
  ...
  cola._SelectFeature = (function(superClass) {
    ...
    _SelectFeature.prototype.init = function(domBinding, force) {
      var feature;
      _SelectFeature.__super__.init.call(this, domBinding, force);
      if (!this.prepared) {
        return;
      }
      feature = this;
  ...
  cola._DomBinding = (function() {
    function _DomBinding(dom, scope1, features, forceInit1, clone) {
      var f, l, len1;
      this.scope = scope1;
    ...
    _DomBinding.prototype.addFeature = function(feature, forceInit) {
      if (feature.id == null) {
        feature.id = cola.uniqueId();
      }
      feature.init(this, forceInit || this.forceInit);
      if (!this.features) {
    ...
    _DomBinding.prototype.bind = function(path, feature) {
      ...
      pipe = {
        path: path,
        processMessage: (function(_this) {
          return function(bindingPath, path, type, arg) {
            if (!feature.disabled) {
              feature.processMessage(_this, bindingPath, path, type, arg);
              if (feature.disabled) {
                pipe.disabled = true;
              }
            } else {
  ...
  cola._RepeatDomBinding = (function(superClass) {
    ...
    function _RepeatDomBinding(dom, scope, feature, forceInit, clone) {
      var f, headerNode, l, len1, repeatItemDomBinding;
      if (clone) {
        _RepeatDomBinding.__super__.constructor.call(this, dom, scope, feature, forceInit, clone);
      } else {
  ...
  cola._RepeatItemDomBinding = (function(superClass) {
    ...
    _RepeatItemDomBinding.prototype.processDataMessage = function(path, type, arg) {
      if (!this.isTemplate) {
        this.scope.data.processMessage("**", path, type, arg);
      }
    };
  ...
  _doRenderDomTemplate = function(dom, scope, context) {
    var aa, attr, attrName, attrValue, base, bindingExpr, bindingType, builder, child, childContext, customDomCompiler, defaultPath, domBinding, f, feature, features, initializer, initializers, k, l, len1, len2, len3, len4, len5, len6, len7, len8, o, oldScope, parts, q, ref, ref1, ref2, ref3, removeAttr, removeAttrs, result, tailDom, u, v, x, y, z;
        ...
        if (attrValue) {
          ...
          if (customDomCompiler) {
            ...
            if (attrName.indexOf("on") === 0) {
              ...
              feature = (builder || cola._domFeatureBuilder["$"]).call(cola._domFeatureBuilder, scope, attrValue, attrName, dom);
            }
            if (feature) {
              if (features == null) {
                features = [];
    ...
    if (features != null ? features.length : void 0) {
      domBinding = cola._domBindingBuilder[bindingType || "$"](dom, scope, features, context);
      if (scope.data.alias) {
        defaultPath = scope.data.alias;
      }
    }
    if (domBinding != null ? domBinding.scope : void 0) {
      oldScope = scope;
      scope = domBinding.scope;
    }
    if (initializers) {
  ...
  cola._domFeatureBuilder = {
    $: function(scope, attrValue, attrName, dom) {
      var feature;
      if (attrName === "display") {
    ...
    repeat: function(scope, attrValue) {
      return new cola._RepeatFeature(attrValue, "repeat");
    },
    alias: function(scope, attrValue) {
      return new cola._AliasFeature(attrValue, "alias");
    },
    bind: function(scope, attrValue, attrName, dom) {
    ...
    style: function(scope, attrValue) {
      var feature, features, style, styleExpr, styleProp;
      if (!attrValue) {
        return false;
      }
    ...
    classname: function(scope, attrValue) {
      var classConfig, classExpr, className, feature, features;
      if (!attrValue) {
        return false;
      }
      features = [];
    ...
    resource: function(scope, attrValue, attrName, dom) {
      attrValue = cola.util.trim(attrValue);
      if (attrValue) {
        $fly(dom).text(cola.resource(attrValue));
      }
    },
    watch: function(scope, attrValue) {
    ...
    event: function(scope, attrValue, attrName) {
  ...
  _compileWidgetDom = function(scope, dom, widgetType, config, context) {
  ...
  cola.WidgetDataModel = (function(superClass) {
    ...
    WidgetDataModel.prototype.get = function(path, loadMode, context) {
      ...
      if (cc === 64) {
        ...
        return this.widget.get(path, true);
      }
    };
  ...
  cola.WidgetModel = (function(superClass) {
    ...
    WidgetModel.prototype._getAction = function(name) {
      ...
      if (method instanceof Function) {
        fn = function() {
          return method.apply(this.widget, arguments);
        };
      }
      if (fn == null) {
        fn = (ref = this.parent) != null ? ref._getAction(name) : void 0;
    ...
    WidgetModel.prototype.processMessage = function(bindingPath, path, type, arg) {
      ...
      return this.data.processMessage(bindingPath, path, type, arg);
    };
...
(function() {
  ...
  cola.TemplateSupport = {
    ...
    trimTemplate: function(template) {
      var c, child, html, k, ref, templs, widgetConfigs;
      if (template.nodeType) {
  ...
  cola.DataWidgetMixin = {
    ...
    _bindSetter: function(bindStr) {
      var bindInfo, expression, i, len1, len2, n, o, p, path, paths, ref, watchingPaths;
      ...
      if (bindStr && this._scope) {
        ...
        bindInfo.watchingMoreMessage = !paths && expression.hasComplexStatement && !expression.hasDefinedPath;
        if (paths) {
          this._watchingPaths = watchingPaths = [];
          for (i = o = 0, len2 = paths.length; o < len2; i = ++o) {
            p = paths[i];
            this._scope.data.bind(p, this._bindProcessor);
  ...
  cola.DataItemsWidgetMixin = {
    ...
    _bindSetter: function(bindStr) {
      ...
      if (bindStr) {
        expression = cola._compileExpression(this._scope, bindStr, "repeat");
        ...
        if (expression.writeable) {
          this._simpleBindPath = expression.writeablePath;
        }
      }
  ...
  cola.Button = (function(superClass) {
    ...
    Button.prototype._parseDom = function(dom) {
      ...
      if (!this._disabled && this.get$Dom().hasClass("disabled")) {
        this._disabled = true;
      }
    };
  ...
  cola.IFrame = (function(superClass) {
    ...
    IFrame.prototype._replaceUrl = function(url) {
      var contentWindow;
      if (this._doms && this._doms.dimmer) {
      ...
      if (contentWindow) {
        ...
        this._doms.iframe && $fly(this._doms.iframe).prop("src", url);
      }
      return this;
    };
  ...
  cola.SubView = (function(superClass) {
    ...
    SubView.prototype.loadIfNecessary = function(options, callback) {
      ...
      if (this._currentUrl && (!options || this._currentUrl === options.url && this._currentJsUrl === options.jsUrl && this._currentCssUrl === options.cssUrl)) {
        cola.callback(callback, true);
      } else {
  ...
  cola.Reveal = (function(superClass) {
    ...
    Reveal.prototype._parseDom = function(dom) {
      ...
      while (child) {
        ...
        results.push(child = child.nextElementSibling);
      }
      return results;
    };
  ...
  cola.ToolbarButton = (function(superClass) {
    extend(ToolbarButton, superClass);

    function ToolbarButton() {
      return ToolbarButton.__super__.constructor.apply(this, arguments);
    }

    ToolbarButton.tagName = "c-toolbar-button";

    ToolbarButton.CLASS_NAME = "toolbar-button";
    ...
    ToolbarButton.prototype._initDom = function(dom) {
      this._doms = {};
      return $(dom).append($.xCreate({
        tagName: "div",
        contextKey: "innerDom",
        "class": "ui button"
      }, this._doms));
    ...
    ToolbarButton.prototype._refreshIcon = function() {
      var $innerDom, caption, icon, iconDom, iconPosition, pos;
      if (!this._dom) {
        return;
      }
      icon = this.get("icon");
      ...
      if (this._icon) {
        ...
        pos = iconPosition === "right" ? "append" : "prepend";
    ...
    ToolbarButton.prototype._doRefreshDom = function() {
      var $innerDom, caption, captionDom, classNamePool;
      if (!this._dom) {
        return;
      }
      ToolbarButton.__super__._doRefreshDom.call(this);
      $innerDom = $(this._doms.innerDom);
      classNamePool = this._classNamePool;
      caption = this._caption;
      captionDom = $innerDom.find(".caption");
      if (caption) {
        if (!captionDom.length) {
          captionDom = $.xCreate({
            tagName: "span",
            "class": "caption"
          });
      ...
      if (this.get("focusable")) {
        $innerDom.attr("tabindex", "0");
      } else {
        $innerDom.removeAttr("tabindex");
      }
      this._refreshIcon();
      $innerDom.toggleClass("disabled", this._disabled);
    };
  ...
  cola.Toolbar = (function(superClass) {
    ...
    Toolbar.tagName = "c-toolbar";

    Toolbar.CLASS_NAME = "ui toolbar";
  ...
  cola.SplitPane = (function(superClass) {
    extend(SplitPane, superClass);

    function SplitPane() {
      return SplitPane.__super__.constructor.apply(this, arguments);
    }

    SplitPane.tagName = "c-split-pane";

    SplitPane.CLASS_NAME = "ui split-pane";

    SplitPane.attributes = {
      direction: {
        refreshDom: true,
        "enum": ["left", "right", "top", "bottom"],
        defaultValue: "top"
    ...
    SplitPane.prototype._doRefreshDom = function() {
      if (!this._dom) {
        return;
      }
      SplitPane.__super__._doRefreshDom.call(this);
      this._setPosition(this._position);
    };

    SplitPane.prototype._setPosition = function(position) {
      var $mainDom, $sideDom, $splitterDom, direction, mainPos;
      direction = this._direction;
      $sideDom = $(this._doms.sideDom);
      $mainDom = $(this._doms.mainDom);
      $splitterDom = $(this._doms.splitter);
    ...
    SplitPane.prototype._initDom = function(dom) {
      var $dom, createMouseMove, doms, fixedHorizontalHandler, fixedVerticalHandler, minHeight, minSize, minWidth, pagePos, pageXof, pageYof, splitPane;
      SplitPane.__super__._initDom.call(this, dom);
      $dom = $(dom);
      doms = this._doms;
      if (!doms.splitter) {
      ...
      pagePos = function(event, pos) {
        var attrName;
        attrName = "page" + pos;
        if (event[attrName] !== void 0) {
          return event[attrName];
        } else if (event.originalEvent[attrName] !== void 0) {
          return event.originalEvent[attrName];
        } else if (event.originalEvent.touches) {
          return event.originalEvent.touches[0][attrName];
        }
      };
      pageXof = function(event) {
        return pagePos(event, "X");
      };
      pageYof = function(event) {
        return pagePos(event, "Y");
      };
      splitPane = this;
      minSize = function(element, name) {
        return parseInt($(element).css('min-' + name), 10) || 0;
      };
      minWidth = function(element) {
        return minSize(element, "width");
      };
      minHeight = function(element) {
        return minSize(element, "height");
      };
      fixedVerticalHandler = function(pageX) {
        var leftOffset, sideMaxWidth, sideMinWidth;
      ...
      fixedHorizontalHandler = function(pageY) {
        var sideMaxHeight, sideMinHeight, topOffset;
      ...
      createMouseMove = function($splitPane, pageX, pageY) {
        var direction;
      ...
      $(doms.splitter).on('mousedown', function() {
        var $splitPane, $splitter, moveEventHandler;
        event.preventDefault();
        $splitter = $(this);
        ...
        return $(document).one('mouseup', function(event) {
          ...
          return $splitter.removeClass('dragged touch');
        });
      });
      return this;
    };

    SplitPane.prototype.destroy = function() {
      if (this._destroyed) {
        return;
      }
      return SplitPane.__super__.destroy.call(this);
    };

    SplitPane.prototype._parseDom = function(dom) {
      var $dom, mainIndex, panes, sideIndex, splitter;
      if (this._doms == null) {
        this._doms = {};
      }
      $dom = $(dom);
      ...
      sideIndex = 0;
      mainIndex = 1;
      if (!(this._direction === "left" || this._direction === "top")) {
        sideIndex = 1;
        mainIndex = 0;
      }
      this._doms.sideDom = panes[sideIndex];
      this._doms.mainDom = panes[mainIndex];
      splitter = $dom.find(">.splitter");
      if (splitter.length) {
        this._doms.splitter = splitter[0];
      }
    };
  ...
  cola.AbstractEditor = (function(superClass) {
    ...
    AbstractEditor.prototype._processDataMessage = function(path, type, arg) {
      var entity, keyMessage, ref, value;
      if (type === cola.constants.MESSAGE_VALIDATION_STATE_CHANGE || (cola.constants.MESSAGE_REFRESH <= type && type <= cola.constants.MESSAGE_CURRENT_CHANGE)) {
        if ((ref = this._bindInfo) != null ? ref.writeable : void 0) {
  ...
  cola.AbstractCheckbox = (function(superClass) {
    ...
    AbstractCheckbox.prototype.post = function() {
      var entity, ref;
      if ((ref = this._bindInfo) != null ? ref.writeable : void 0) {
        entity = this._scope.get(this._bindInfo.entityPath);
        if (entity instanceof cola.EntityList) {
        ...
        if (entity) {
          AbstractCheckbox.__super__.post.call(this);
        }
      }
      return this;
    };
    ...
    Input.prototype._initDom = function(dom) {
      ...
      $(this._doms.input).on("change", (function(_this) {
        ...
        return function() {
          ...
          if ((_this._value == null) || _this._value === "" && ((ref = _this._bindInfo) != null ? ref.writeable : void 0)) {
  ...
  cola.RadioGroup = (function(superClass) {
    ...
    RadioGroup.prototype._doRefreshDom = function() {
      ...
      if (itemsDom) {
        $fly(this._dom).empty().append(itemsDom);
      }
      value = this._value;
    ...
    RadioGroup.prototype._select = function(value) {
      ...
      dom = $(this._dom).find("[value='" + value + "']");
      if (dom.length > 0) {
        return dom[0].checked = true;
      }
    };

    RadioGroup.prototype._getDomValue = function(itemDom) {
      var item;
    ...
    RadioGroup.prototype._getItemsDom = function() {
      var attrBinding, itemsDom, raw, ref, textProperty, valueProperty;
  ...
  cola.findDropDown = function(target) {
    ...
    while (layer && !layer.hasClass("drop-container")) {
      layer = cola.findWidget(layer, cola.AbstractLayer);
    }
    return layer != null ? layer._dropdown : void 0;
  };
  ...
  cola.AbstractDropdown = (function(superClass) {
    ...
    AbstractDropdown.prototype._getContainer = function() {
      ...
      if (this._container) {
        ...
        this._dropdownContent = config.dom.firstChild;
    ...
    Dropdown.TEMPLATES = {
      ...
      "list": {
        ...
        keydown: function(evt) {
          if (!this._disableKeyBubble) {
            this._disableKeyBubble = true;
            $fly(this).find(">c-listview").trigger(evt);
            this._disableKeyBubble = false;
          }
          return true;
        }
      },
      "filterable-list": {
        ...
        keydown: function(evt) {
          if (!this._disableKeyBubble) {
            this._disableKeyBubble = true;
            $fly(this).find(">.flex-box >c-listview").trigger(evt);
            this._disableKeyBubble = false;
          }
          return true;
        }
  ...
  cola.Form = (function(superClass) {
    ...
    Form.prototype.refreshMessages = function() {
      ...
      messageHolder = this._messageHolder;
      messageHolder.clear();
      fieldDoms = this._dom.querySelectorAll("field");
      for (n = 0, len1 = fieldDoms.length; n < len1; n++) {
        fieldDom = fieldDoms[n];
        field = cola.widget(fieldDom);
        if (field != null ? field._message : void 0) {
      ...
      this._$dom.removeClass("error warning success").addClass(state);
      this._$messages.removeClass("error warning success").addClass(state).empty();
      if (messageCosons.length > 0) {
        this._$messages.xAppend({
          tagName: "ul",
          "class": "list",
  ...
  cola.Field = (function(superClass) {
    ...
    Field.prototype._parseDom = function(dom) {
      ...
      if (!this._bind && this._property) {
        if (dom.parentNode) {
          if (dom.parentNode.nodeName === "C-FORM") {
            this._formDom = dom.parentNode;
          } else if (((ref = dom.parentNode.parentNode) != null ? ref.nodeName : void 0) === "C-FORM") {
            this._formDom = dom.parentNode.parentNode;
          }
        }
        if (this._formDom) {
          this._form = cola.widget(this._formDom);
          formBind = (ref1 = this._form) != null ? ref1._bind : void 0;
          if (formBind) {
            bind = formBind + "." + this._property;
          } else {
            bind = this._property;
          }
        }
    ...
    Field.prototype._filterDataMessage = function(path, type, arg) {
      return type === cola.constants.MESSAGE_VALIDATION_STATE_CHANGE || cola.constants.MESSAGE_REFRESH;
    };

    Field.prototype._processDataMessage = function(path, type, arg) {
      var entity, keyMessage, ref;
      if (type === cola.constants.MESSAGE_VALIDATION_STATE_CHANGE || cola.constants.MESSAGE_REFRESH) {
        if ((ref = this._bindInfo) != null ? ref.writeable : void 0) {
          entity = this._scope.get(this._bindInfo.entityPath);
          if (entity instanceof cola.EntityList) {
            entity = entity.current;
  ...
  cola.ItemsView = (function(superClass) {
    ...
    ItemsView.prototype._getFirstItemDom = function() {
      var itemDom;
      itemDom = this._doms.itemsWrapper.firstElementChild;
      while (itemDom) {
  ...
  cola.Carousel = (function(superClass) {
    ...
    Carousel.prototype._initDom = function(dom) {
      var carousel, refreshItems, template;
      ...
      if (template) {
        ...
        cola.xRender(template, this._scope);
        refreshItems = function(ev) {
          carousel._refreshItemsTimer && clearTimeout(carousel._refreshItemsTimer);
          return carousel._refreshItemsTimer = setTimeout(function() {
            return carousel.refreshItems();
          }, 100);
        };
        this._doms.wrap.addEventListener("DOMNodeRemoved", refreshItems, false);
        this._doms.wrap.addEventListener("DOMNodeInserted", refreshItems, false);
      }
  ...
  cola.Menu = (function(superClass) {
    ...
    Menu.prototype._parseItems = function(node) {
      ...
      while (childNode) {
        ...
        results.push(childNode = childNode.nextElementSibling);
      }
      return results;
    };
  ...
  cola.TitleBar = (function(superClass) {
    ...
    TitleBar.prototype._parseDom = function(dom) {
      ...
      if (this._doms.title && dom.firstChild !== this._doms.title) {
  ...
  cola.ListView = (function(superClass) {
    ...
    ListView.prototype._onItemsWrapperScroll = function() {
      ...
      if (topGroupDom) {
        ...
        gap = 1;
        nextOffsetTop = (ref2 = topGroupDom.nextElementSibling) != null ? ref2.offsetTop : void 0;
    ...
    ListView.prototype._findTopGroupDom = function(scrollTop) {
      ...
      currentGroupDom = this._topGroupDom || this._doms.itemsWrapper.firstElementChild;
    ...
    ListView.prototype._onItemsWrapperTouchMove = function(evt) {
      ...
      if (itemDom.firstElementChild && itemDom.firstElementChild === itemDom.lastChild) {
    ...
    CascadeBind.attributes = {
      expression: {
        setter: function(expression) {
          expression = cola._compileExpression(this._scope, expression, "repeat");
          if (expression) {
            if (!expression.repeat) {
    ...
    CascadeBind.prototype.retrieveChildNodes = function(parentNode, callback, dataCtx) {
      ...
      if (this._recursive || isRoot) {
        ...
        if (items === void 0 && dataCtx.unloaded) {
          ...
          if (items instanceof Array) {
            originRecursiveItems = items.$origin;
          }
          if (recursiveItems) {
        ...
        cola.util.waitForAll(funcs, {
          ...
          complete: function(success, result) {
            ...
            if (success) {
              hasChild = false;
              if (this._recursive || isRoot) {
                recursiveItems = this._expression.evaluate(parentNode._scope, "never");
                if (recursiveItems instanceof Array) {
  ...
  cola.AbstractTable = (function(superClass) {
        ...
        if (column instanceof cola.TableGroupColumn) {
          ...
          if (column._bind) {
            ...
            if (bind.charCodeAt(0) === 46) {
              ...
              info.expression = cola._compileExpression(this._scope, bind);
            }
          }
          if (column._width) {
            width = column._width;
    ...
    AbstractTable.prototype._parseDom = function(dom) {
      var child, column, columns, next, nodeName;
      if (!dom) {
        return;
      }
      if (this._doms == null) {
    ...
    AbstractTable.prototype._parseColumnDom = function(dom) {
      var attr, attrName, child, column, expression, len1, n, next, nodeName, ref, subColumn, templateName;
      column = {};
      ref = dom.attributes;
      for (n = 0, len1 = ref.length; n < len1; n++) {
        attr = ref[n];
        attrName = attr.name;
        if (attrName.substring(0, 2) === "c-") {
          expression = cola._compileExpression(this._scope, attr.value);
      ...
      while (child) {
        ...
        if (nodeName === "TEMPLATE") {
          ...
          if (templateName && templateName.indexOf("header") === 0) {
            templateName = "headerTemplate";
          } else if (templateName && templateName.indexOf("footer") === 0) {
  ...
  cola.Table = (function(superClass) {
    ...
    Table.prototype._getFixedHeader = function(create) {
      ...
      if (!fixedHeaderWrapper && create) {
        ...
        this._refreshFakeRow(fakeThead.firstElementChild);
      }
    ...
    Table.prototype._getFixedFooter = function(create) {
      ...
      if (!fixedFooterWrapper && create) {
        ...
        this._refreshFakeRow(fakeTfoot.firstElementChild);
      }
    ...
    Table.prototype._setFixedHeaderSize = function() {
      ...
      $fly(this._doms.fakeThead.firstElementChild).height(this._doms.thead.offsetHeight);
    };
    ...
    Table.prototype._setFixedFooterSize = function() {
      ...
      $fly(this._doms.fakeTfoot.firstElementChild).height(this._doms.tfoot.offsetHeight);
    };
    ...
    Table.prototype._refreshFixedHeader = function() {
      ...
      showFixedHeader = scrollTop > 0 && !(cola.browser.ie === 11);
      ...
      if (showFixedHeader) {
        ...
        if (this._doms.fakeThead.parentNode) {
          this._doms.fixedHeaderTable.removeChild(this._doms.fakeThead);
        }
        ...
        if (fixedHeader) {
          ...
          this._doms.fixedHeaderTable.removeChild(this._doms.thead);
    ...
    Table.prototype._refreshFixedFooter = function(duration) {
      ...
      showFixedFooter = scrollTop < maxScrollTop && !(cola.browser.ie === 11);
      ...
      if (showFixedFooter) {
        ...
        if (this._doms.fakeTfoot.parentNode) {
          this._doms.fixedFooterTable.removeChild(this._doms.fakeTfoot);
        }
        ...
        if (fixedFooter) {
          ...
          this._doms.fixedFooterTable.removeChild(this._doms.tfoot);
  ...
  cola.Pager = (function(superClass) {
    ...
    Pager.CLASS_NAME = "ui menu pager secondary";
    ...
    function Pager(config) {
      ...
      this._pagerItemConfig = {
        ...
        pageSize: {
          ...
          change: function() {
            ...
            if (collection) {
              ...
              if (value === collection.pageSize) {
                return;
              }
              if (collection instanceof cola.EntityList) {
                invoker = collection._providerInvoker;
                invoker.ajaxService.set("pageSize", value);
                return collection.pageSize = value;
              }
            }
          }
        },
        goto: {
          ...
          change: function() {
            var $input, button, buttonDom, data, pageCount, pageNo, value;
    ...
    Pager.prototype.pagerItemsRefresh = function() {
      var data, descDom, gotoInput, gotoInputControl, gotoItem, hasNext, hasPrev, infoItem, infoItemDom, pageCount, pageNo, pageSize, pageSizeDom, pageSizeInput, pageSizeItem, pager, ref, ref1, ref2, ref3, ref4, totalEntityCount;
      pager = this;
      data = pager._getBindItems();
      hasPrev = false;
      hasNext = false;
